...
if(isset($_POST['add'])){
   ...
   $photo_tmp=$_FILES['photo']['tmp_name'];
    ...
    $photo_query="select * from student_details where sid =
     '$edit_st_id'";

     $run=mysqli_query($conn,$photo_query);
     
	while ($row = mysqli_fetch_assoc($run)) {
		$img=$row['photo'];
	}

    unlink('studentphoto/'.$img);
    ...
    $sql="update student_details set fname =' $fname', lname ='$lname', email='$email', birthhdate='$birthhdate',dept ='$dept',
    gender = '$gender', $='$mobile', photo=' $photo' where sid='$edit_st_id'";
	$run=mysqli_query($conn,$sql);
    if($run){
    echo  "<script>window.open('viewstudent.php?update_success=updated','_self') </script>";
		}
    else{
    echo  "<script>window.open('viewstudent.php?update_error = not updated','_self') </script>";
		}
